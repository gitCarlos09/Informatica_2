...
print("El par más cercano son los puntos", parCercano, "con una distancia de", distanciaMinima)
...
diccCine = {
    "2D": {"NIÑOS": {"EntreSemana": 4000, "FinSemana": 6000}, "ADULTOS": {"EntreSemana": 7000, "FinSemana": 8000}},
    "3D": {"NIÑOS": {"EntreSemana": 7000, "FinSemana": 8000}, "ADULTOS": {"EntreSemana": 11000, "FinSemana": 14000}}
}
...
for j in lista_Ventas:
    ventasÑinos=j[0]    
    ventasAdultos=j[1]

    formatoNiños=ventasÑinos[0:2]   
    formatoAdultos=ventasAdultos[0:2]

    boletasNiños=int(ventasÑinos[3])   
    boletasAdultos=int(ventasAdultos[3]) 

    sumaBoletaNiño+=boletasNiños      
    sumaBoletaAdulto+=boletasAdultos

    semanaNiños=ventasÑinos[10:]         
    semanaAdultos=ventasAdultos[12:]

    EsNiño=ventasÑinos[4:9]     
    EsAdulto=ventasAdultos[4:11]

    if semanaNiños in Semana:
        semanaNiños="EntreSemana"      
    else:
        semanaNiños="FinSemana"        
        
    if semanaAdultos in Semana:
        semanaAdultos="EntreSemana"
    else:
        semanaAdultos="FinSemana"  

    precioNiños=diccCine[formatoNiños][EsNiño][semanaNiños]         
    precioAdultos=diccCine[formatoAdultos][EsAdulto][semanaAdultos]

    dinero_Niños+=precioNiños*boletasNiños            
    dinero_Adultos+=precioAdultos*boletasAdultos      

    dineroRecaudado=dinero_Niños+dinero_Adultos

boletasVendidas=sumaBoletaAdulto+sumaBoletaNiño    
...
def obtenerNesimoFibonacci (numero_pedido):
    fibo = 1
    back = -1
...
ventas = ["A032-52Unidades", "B001-29Unidades", "A125-15Unidades", "A032-22Unidades", "P009-25Unidades",
...
def calcularVentas(ventas):
    ...
    for Cod in ventas:
       codigo1=Cod[0:4]
       unidades=int(Cod[5:7] )
       precio=dicc_Precio[codigo1]
       precioPorVenta=precio*unidades
       DineroRecaudado+=precioPorVenta
    return DineroRecaudado

print(calcularVentas(ventas))
...
def calcularSalario(nombreEmpleado,unidadesVendidas):
    dicc={}
    dicc["nombre"]=nombreEmpleado
    salBase=1_200_000

    Comision1=50_000*0.05
    Comision2=70_000*0.04
    Comision3=40_000*0.06
    Comision4=25_000*0.07
    Comision5=35_000*0.05
    Comision6=80_000*0.03
    Comision7=95_000*0.02

    comisiones=[Comision1,Comision2,Comision3,Comision4,Comision5,Comision6,Comision7]

    sum=0
    salariototal=0
    for k in unidadesVendidas:
        totalArt=k*comisiones[sum]
        salariototal+=totalArt
        sum+=1
        
    dicc["salario"]=int(salariototal+salBase)
    return dicc

print(calcularSalario("Jhanth Carlo",[27,19,9,31,16,20,18]))